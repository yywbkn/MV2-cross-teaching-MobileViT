...
from networks.Mobilenet_base_seg import MobileBaseSeg
from utils import losses, metrics, ramps
from val_2D import test_single_volume,test_single_volume_covid

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str,default='../data/COVID', help='Name of Experiment')
parser.add_argument('--exp', type=str,default='COVID/Cross_Teaching_Between_CNN_Transformer', help='experiment_name')
parser.add_argument('--model', type=str, default='MobileViT', help='model_name')
...
def train(args, snapshot_path):
    ...
    model1 = create_model()
    model2 = MobileBaseSeg(in_chns=1, class_num=num_classes).cuda()

    def worker_init_fn(worker_id):
        random.seed(args.seed + worker_id)

    db_train = BaseDataSets(base_dir=args.root_path, split="train", num=None, transform=transforms.Compose([ RandomGenerator(args.patch_size) ]))
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_volume_covid(sampled_batch["image"], sampled_batch["label"], model1, classes=num_classes, patch_size=args.patch_size)
                ...
                metric_list = 0.0
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_volume_covid( sampled_batch["image"], sampled_batch["label"], model2, classes=num_classes, patch_size=args.patch_size)
                    metric_list += np.array(metric_i)
                metric_list = metric_list / len(db_val)
                for class_i in range(num_classes-1):
